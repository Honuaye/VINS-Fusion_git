...
void publish_car_model(double t,
                       Eigen::Vector3d t_w_car,
...
void vio_callback(const nav_msgs::Odometry::ConstPtr &pose_msg) {
    ...
    while (!gpsQueue.empty()) {
        ...
        if (gps_t >= t - 0.01 && gps_t <= t + 0.01) {
            ...
            globalEstimator.inputGPS(t, latitude, longitude, altitude,
                                     pos_accuracy);
            gpsQueue.pop();
            break;
...
int main(int argc, char **argv) {
    ...
    pub_global_odometry =
        n.advertise<nav_msgs::Odometry>("global_odometry", 100);
