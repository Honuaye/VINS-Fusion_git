...
void ResidualBlockInfo::Evaluate() {
    ...
    for (int i = 0; i < static_cast<int>(block_sizes.size()); i++) {
        ...
        // raw_jacobians[i] 地址应该是指向了jacobian[i]
...
void MarginalizationInfo::addResidualBlockInfo(
    ...
    for (int i = 0; i < static_cast<int>(residual_block_info->parameter_blocks.size());
         i++) {
...
// 而边缘化采取了舒尔补的做法，将需要边缘化的变量从Ax=b的参数矩阵中剥离，
// 并且重新构造雅可比矩阵与误差。
// 值得一提的是，在求解ba问题（例如用g2o的求解）时，
// 通常会使用边缘化消掉路标点，也是用舒尔补的方法，但那是先计算位姿再计算路标点，并没有将路标点真正抛弃。
        ...
        if (size != 7)
            ...
            if (!((Eigen::Quaterniond(x0(6), x0(3), x0(4), x0(5)).inverse() *
                ...
                dx.segment<3>(idx + 3) =
                    2.0 *
                    -Utility::positify(
                         Eigen::Quaterniond(x0(6), x0(3), x0(4), x0(5))
                             .inverse() *
                         Eigen::Quaterniond(x(6), x(3), x(4), x(5)))
                         .vec();
            }
    ...
    if (jacobians) {
        for (int i = 0; i < static_cast<int>(marginalization_info->keep_block_size.size()); i++) {
            if (jacobians[i]) {
                ...
                jacobian.setZero();
                // middleCols : P.middleCols<cols>(j)              // P(:, j+1:j+cols)中间从j数cols列
