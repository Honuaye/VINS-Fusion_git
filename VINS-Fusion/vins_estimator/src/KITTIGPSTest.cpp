...
int main(int argc, char** argv) {
    ...
    if (argc != 3) {
        printf(
            "please intput: rosrun vins kitti_gps_test [config file] [data "
            "folder] \n"
            "for example: rosrun vins kitti_gps_test "
            "~/catkin_ws/src/VINS-Fusion/config/kitti_raw/"
            "kitti_10_03_config.yaml "
            "/media/tony-ws1/disk_D/kitti/2011_10_03/"
    ...
    for (size_t i = 0; i < imageTimeList.size(); i++) {
        if (ros::ok()) {
            ...
            // vu);
            fscanf(GPSFile, "%lf %lf %lf %lf %lf %lf ", &ax, &ay, &az, &af, &al,
                   &au);
            // printf("ax:%lf ay:%lf az:%lf af:%lf al:%lf au:%lf\n",  ax, ay,
            // az, af, al, au);
            fscanf(GPSFile, "%lf %lf %lf %lf %lf %lf ", &wx, &wy, &wz, &wf, &wl,
                   &wu);
            // printf("wx:%lf wy:%lf wz:%lf wf:%lf wl:%lf wu:%lf\n",  wx, wy,
            // wz, wf, wl, wu);
            fscanf(GPSFile, "%lf %lf %lf %lf %lf %lf ", &pos_accuracy,
