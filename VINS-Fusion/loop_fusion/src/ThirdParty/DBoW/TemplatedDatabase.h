...
class TemplatedDatabase {
 public:
     ...
     */
    template <class T>
    explicit TemplatedDatabase(const T &voc,
                               bool use_di = true,
                               int di_levels = 0);
     ...
     *   < 0 means all
     */
    void query(const std::vector<TDescriptor> &features,
               QueryResults &ret,
               int max_results = 1,
               int max_id = -1) const;
 ...
 protected:
    ...
    void queryL1(const BowVector &vec,
                 QueryResults &ret,
                 int max_results,
                 int max_id) const;

    /// Query with L2 scoring
    void queryL2(const BowVector &vec,
                 QueryResults &ret,
                 int max_results,
                 int max_id) const;

    /// Query with Chi square scoring
    void queryChiSquare(const BowVector &vec,
                        QueryResults &ret,
                        int max_results,
                        int max_id) const;

    /// Query with Bhattacharyya scoring
    void queryBhattacharyya(const BowVector &vec,
                            QueryResults &ret,
                            int max_results,
                            int max_id) const;

    /// Query with KL divergence scoring
    void queryKL(const BowVector &vec,
                 QueryResults &ret,
                 int max_results,
                 int max_id) const;

    /// Query with dot product scoring
    void queryDotProduct(const BowVector &vec,
                         QueryResults &ret,
                         int max_results,
                         int max_id) const;
...
template <class TDescriptor, class F>
template <class T>
TemplatedDatabase<TDescriptor, F>::TemplatedDatabase(const T &voc,
                                                     bool use_di,
                                                     int di_levels)
    ...
    for (vit = vec.begin(); vit != vec.end(); ++vit) {
        ...
        for (rit = row.begin(); rit != row.end(); ++rit) {
            ...
            if ((int)entry_id < max_id || max_id == -1 ||
                ...
                double value =
                    fabs(qvalue - dvalue) - fabs(qvalue) - fabs(dvalue);
...
void TemplatedDatabase<TDescriptor, F>::queryL2(const BowVector &vec,
    ...
    for (vit = vec.begin(); vit != vec.end(); ++vit) {
        ...
        for (rit = row.begin(); rit != row.end(); ++rit) {
            ...
            if ((int)entry_id < max_id || max_id == -1) {
                double value =
                    -qvalue * dvalue;  // minus sign for sorting trick
...
void TemplatedDatabase<TDescriptor, F>::queryBhattacharyya(const BowVector &vec,
    ...
    std::map<EntryId, std::pair<double, int> >
        pairs;  // <eid, <score, counter> >
    std::map<EntryId, std::pair<double, int> >::iterator pit;
...
void TemplatedDatabase<TDescriptor, F>::save(cv::FileStorage &fs,
    ...
    for (iit = m_ifile.begin(); iit != m_ifile.end(); ++iit) {
        ...
        for (irit = iit->begin(); irit != iit->end(); ++irit) {
            fs << "{:"
               << "imageId" << (int)irit->entry_id << "weight"
               << irit->word_weight << "}";
        }
        fs << "]";  // word of IF
    }
