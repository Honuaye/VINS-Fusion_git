...
void PoseGraph::setIMUFlag(bool _use_imu) {
    ...
    if (use_imu) {
        printf(
            "VIO input, perfrom 4 DoF (x, y, z, yaw) pose graph "
            "optimization\n");
        ...
        if (cur_kf->has_loop) {
            ...
            if (cur_kf->sequence > 0) {
                ...
                posegraph_visualization->add_loopedge(
                    P0,
                    connected_P + Vector3d(VISUALIZATION_SHIFT_X,
...
int PoseGraph::detectLoop(KeyFrame *keyframe, int frame_index) {
    ...
    if (DEBUG_IMAGE) {
        ...
        putText(compressed_image, "feature_num:" + to_string(feature_num),
                cv::Point2f(10, 10), CV_FONT_HERSHEY_SIMPLEX, 0.4,
                cv::Scalar(255));
    ...
    while (true) {
        ...
        if (cur_index != -1) {
            ...
            for (it = keyframelist.begin(); it != keyframelist.end(); it++) {
                ...
                for (int j = 1; j < 5; j++) {
                    if (i - j >= 0 &&
                        sequence_array[i] == sequence_array[i - j]) {
                        ...
                        double relative_yaw =
                            euler_array[i][0] - euler_array[i - j][0];
                        ceres::CostFunction *cost_function =
                            FourDOFError::Create(relative_t.x(), relative_t.y(),
                                                 relative_t.z(), relative_yaw,
                                                 euler_conncected.y(),
                                                 euler_conncected.z());
                        problem.AddResidualBlock(
                            cost_function, NULL, euler_array[i - j],
                ...
                if ((*it)->has_loop) {
                    ...
                    int connected_index =
                        getKeyFrame((*it)->loop_index)->local_index;
            ...
            */
            ...
            for (it = keyframelist.begin(); it != keyframelist.end(); it++) {
                if ((*it)->index < first_looped_index) continue;
                Quaterniond tmp_q;
                tmp_q = Utility::ypr2R(Vector3d(
                    euler_array[i][0], euler_array[i][1], euler_array[i][2]));
                Vector3d tmp_t =
                    Vector3d(t_array[i][0], t_array[i][1], t_array[i][2]);
            ...
            // cout << "r_drift " << Utility::R2ypr(r_drift).transpose() <<
            // endl;
            ...
            for (it = keyframelist.begin(); it != keyframelist.end(); it++) {
                ...
                for (int j = 1; j < 5; j++) {
                    if (i - j >= 0 &&
                        ...
                        Quaterniond q_i =
                            Quaterniond(q_array[i][0], q_array[i][1],
                        ...
                        ceres::CostFunction *vo_function =
                            RelativeRTError::Create(
                                relative_t.x(), relative_t.y(), relative_t.z(),
                                relative_q.w(), relative_q.x(), relative_q.y(),
                                relative_q.z(), 0.1, 0.01);
                        problem.AddResidualBlock(vo_function, NULL,
                                                 q_array[i - j], t_array[i - j],
                                                 q_array[i], t_array[i]);
                    }
                }
                ...
                if ((*it)->has_loop) {
                    ...
                    ceres::CostFunction *loop_function =
                        RelativeRTError::Create(relative_t.x(), relative_t.y(),
                                                relative_t.z(), relative_q.w(),
...
void PoseGraph::savePoseGraph() {
    ...
    for (it = keyframelist.begin(); it != keyframelist.end(); it++) {
        ...
        fprintf(pFile,
                " %d %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %d %f %f "
                "%f %f %f %f %f %f %d\n",
...
void PoseGraph::loadPoseGraph() {
    ...
    while (fscanf(pFile,
                  ...
                  "%lf %lf %d %lf %lf %lf %lf %lf %lf %lf %lf %d",
                  &index, &time_stamp, &VIO_Tx, &VIO_Ty, &VIO_Tz, &PG_Tx,
                  &PG_Ty, &PG_Tz, &VIO_Qw, &VIO_Qx, &VIO_Qy, &VIO_Qz, &PG_Qw,
                  &PG_Qx, &PG_Qy, &PG_Qz, &loop_index, &loop_info_0,
                  &loop_info_1, &loop_info_2, &loop_info_3, &loop_info_4,
                  &loop_info_5, &loop_info_6, &loop_info_7,
                  &keypoints_num) != EOF) {
        /*
        printf("I read: %d %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf
        %lf
        %lf %d %lf %lf %lf %lf %lf %lf %lf %lf %d\n", index, time_stamp,
                                    ...
                                    loop_index,
                                    loop_info_0, loop_info_1, loop_info_2,
        loop_info_3,
                                    loop_info_4, loop_info_5, loop_info_6,
        loop_info_7,
                                    keypoints_num);
        */
        cv::Mat image;
        ...
        for (int i = 0; i < keypoints_num; i++) {
            ...
            if (!fscanf(keypoints_file, "%lf %lf %lf %lf", &p_x, &p_y,
                        &p_x_norm, &p_y_norm))
